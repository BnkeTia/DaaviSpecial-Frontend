    ...
    <nav className="fixed z-10 w-full bg-white shadow-lg">
      ...
      <div className={`md:hidden ${nav ? 'block' : 'hidden'} max-md:mb-7 max-md:mr-[15px]`}>
        ...
        {isAuthenticated ? (
          ...
          <div className="flex flex-col space-y-2">
            <NavLink to="/login" className="block px-4 py-2 text-white bg-blue-600 rounded hover:bg-blue-700">Login</NavLink>
            <NavLink to="/register" className="block px-4 py-2 text-white bg-green-600 rounded hover:bg-green-700">Register</NavLink>
          </div>
        )}
      </div>
    </nav>
